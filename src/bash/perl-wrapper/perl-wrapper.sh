#!/bin/bash 
# file: src/bash/xls-to-rdbms.sh doc at the eof file
...
doRunActions(){
   ...
   test -z "$actions" && doPrintUsage && doExit 0 
	...
	while read -d ' ' action ; do (
		...
		done < <(find src/bash/xls-to-rdbms/funcs -type f -name '*.sh')
...
doSetVars(){
   ...
   # stop set default vars
	...
	test -z "$xls_to_rdbms_project" && doParseConfFile
	test -z "$xls_to_rdbms_project" || doSetUndefinedShellVarsFromCnfFile
...
#----------------------------------------------------------
#
#eof file: xls-to-rdbms.sh v1.2.5
