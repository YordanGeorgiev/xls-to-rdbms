#!/bin/bash 
# file: src/bash/xls-to-rdbms/test-xls-to-rdbms.sh
      ...
      doLogTestRunEntry 'START'
		while read -r test_file ; do (
            ...
            then
               ...
               $function_name
               doLog "INFO test-xls-to-rdbms loop exit_code: $exit_code" 
               doLogTestRunEntry 'INFO' ' '$(date "+%H:%M:%S")
		...
		done < <(find src/bash/xls-to-rdbms/tests -type f -name '*.sh')

	);
	done < <(cat $product_instance_dir/src/bash/xls-to-rdbms/tests/run-xls-to-rdbms-tests.lst)
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date "+%Y.%m.%d-%H:%M:%S"` [xls-to-rdbms][@$host_name] [$$] $msg "
...
doSetVars(){
   ...
   . "$product_instance_dir/src/bash/xls-to-rdbms/funcs/log-test-run-entry.func.sh"
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub xls-to-rdbmser
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: src/bash/xls-to-rdbms/test-xls-to-rdbms.sh
