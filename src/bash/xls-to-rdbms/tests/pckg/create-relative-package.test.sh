...
doTestCreateRelativePackage(){
   ...
   exit_code=0
	bash src/bash/xls-to-rdbms/xls-to-rdbms.sh -a create-relative-package
   export exit_code=$?
   ...
   bash src/bash/xls-to-rdbms/xls-to-rdbms.sh -a create-relative-package -i met/.tst.xls-to-rdbms
   export exit_code=$?
   ...
   test $exit_code -ne 0 && return
	...
	bash src/bash/xls-to-rdbms/xls-to-rdbms.sh -a create-relative-package -i met/.prd.xls-to-rdbms
   export exit_code=$?
   ...
   # test the call with the include file of the prd env
   bash src/bash/xls-to-rdbms/xls-to-rdbms.sh -a create-relative-package -i met/.git.xls-to-rdbms
   export exit_code=$?
